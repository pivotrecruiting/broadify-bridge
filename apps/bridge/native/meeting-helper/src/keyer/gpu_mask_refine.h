...
// GPU mask refinement (Stage 2 of the zero-copy rework). Takes the low-res
...
class GpuMaskRefiner {
 public:
  ...
  // next refine call on this instance) — for the fused GPU path that composites
  // without a CPU round-trip. `outWidth`/`outHeight` receive the texture size.
